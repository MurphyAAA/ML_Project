...
class MVG(Models):
    ...
    def validation(self, DTE, LTE):
        logGau0 = MVG.logpdf_GAU_ND(DTE, self.mu[0], self.sigma[0])
        logGau1 = MVG.logpdf_GAU_ND(DTE, self.mu[1], self.sigma[1])

        jointLogDen0 = logGau0 + np.log(2 / 3)
        jointLogDen1 = logGau1 + np.log(1 / 3)

        logSJoint = np.vstack((jointLogDen0, jointLogDen1))
        logSMarginal = MVG.vrow(scipy.special.logsumexp(logSJoint, axis=0))
